package com.example.maha_weather_app
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var rlZipCode: RelativeLayout
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mybtn = findViewById(R.id.btnErrorZipcode)
        ...
        rlZipCode = findViewById(R.id.rlZip)
        etZipCode = findViewById(R.id.etZipcode)
        btZip = findViewById(R.id.btnZipcode)

        btZip.setOnClickListener {
            city = etZipCode.text.toString();
            requestAPI()
            etZipCode.text.clear()

            val imm = ContextCompat.getSystemService(this, InputMethodManager::class.java)
            imm?.hideSoftInputFromWindow(this.currentFocus?.windowToken, 0)
            rlZipCode.isVisible = false
        }
    ...
    private suspend fun updateWeatherData(result: String){
        withContext(Dispatchers.Main){
            ...
            findViewById<TextView>(R.id.tvAddress).setOnClickListener {
                rlZipCode.isVisible = true
            }
            findViewById<TextView>(R.id.tvLastUpdate).text =  lastUpdateText
